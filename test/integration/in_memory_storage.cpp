...
class InMemoryStorage : public gqe::test::BaseFixture {
 protected:
  InMemoryStorage() : ctx_ref{get_task_manager_ctx(), get_query_ctx()} {}
...
TEST_F(InMemoryStorage, CopyTable)
{
  ...
  // Verify write statistics were collected in the task graph.
  ASSERT_NE(task_graph->write_statistics, nullptr);
  catalog.statistics("out_table")
