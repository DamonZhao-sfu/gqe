...
TEST(RelationPropertiesUniqueKeys, CheckUniqueKeyTrue)
{
  props_t props;
  props.add_unique_key({0, 1});
  EXPECT_TRUE(props.check_unique_key({0, 1}));
}

TEST(RelationPropertiesUniqueKeys, CheckUniqueKeyFalse)
{
  props_t props;
  props.add_unique_key({0, 1});
  EXPECT_FALSE(props.check_unique_key({2, 3}));
}

TEST(RelationPropertiesUniqueKeys, CheckUniqueKeyOrderInsensitive)
{
  props_t props;
  props.add_unique_key({0, 1, 2});
  EXPECT_TRUE(props.check_unique_key({2, 1, 0}));
}

TEST(RelationPropertiesUniqueKeys, CheckUniqueKeyEmpty)
{
  props_t props;
  EXPECT_FALSE(props.check_unique_key({0}));
}
