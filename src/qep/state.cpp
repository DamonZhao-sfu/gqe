...
state_container make_empty_state_container(std::span<cudf::data_type const> data_types)
{
  GQE_EXPECTS(!data_types.empty(),
              ...
              "make_count_only_container(0) for column-less zero-row results)");
...
state_container make_count_only_container(cudf::size_type row_count)
{
