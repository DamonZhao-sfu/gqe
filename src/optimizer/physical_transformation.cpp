  ...
  logical::relation const* logical_relation)
{
  ...
  switch (logical_relation->type()) {
    ...
    case logical::relation::relation_type::join: {
      ...
      if (_params && !_params->join_use_unique_keys)
        unique_keys_pol = gqe::unique_keys_policy::none;
      ...
      // Hash map cache: enabled by flag, and additionally always enabled when mark join is used
      // (mark join is itself gated on broadcast-left semi/anti, see above).
      bool const use_hash_map_cache =
        (_params && _params->join_use_hash_map_cache) || use_mark_join;
