...
class projection_pushdown::apply_visitor : public gqe::logical::relation_visitor {
 public:
  apply_visitor(projection_pushdown const& rule, bool& rule_applied)
  ...
  void visit_relation(gqe::logical::relation* rel) override
  {
    ...
    // Check each child: if it's a project-over-(filter/join), push it down.
    auto children = rel->children_safe();
    for (std::size_t i = 0; i < children.size(); ++i) {
      auto& child = children[i];
      if (child->type() != relation_t::project) continue;
      auto project       = std::static_pointer_cast<logical::project_relation>(child);
      auto grandchildren = project->children_safe();
      assert(grandchildren.size() == 1);
      auto& grandchild = grandchildren[0];

      std::shared_ptr<logical::relation> replacement;
      switch (grandchild->type()) {
        case relation_t::filter:
          replacement =
            _rule.try_pushdown<logical::filter_relation>(project, grandchild, _rule_applied);
          break;
        case relation_t::join:
          replacement =
            _rule.try_pushdown<logical::join_relation>(project, grandchild, _rule_applied);
          break;
        default: continue;
      }
      if (replacement != child) { projection_pushdown::replace_child_at(rel, i, replacement); }
    }
  }

 private:
  projection_pushdown const& _rule;
  bool& _rule_applied;
};
...
std::shared_ptr<gqe::logical::relation> gqe::optimizer::projection_pushdown::apply(
  std::shared_ptr<logical::relation> root, bool& rule_applied) const
{
  // Walk the tree: for each node, check children for project-over-(filter/join) patterns.
  apply_visitor visitor{*this, rule_applied};
  root->accept(visitor);

  // Handle the root separately: the visitor checks from the parent, so a root that is itself
  // a project-over-(filter/join) is not handled by the visitor's child-check loop.
  if (root->type() == relation_t::project) {
    auto project  = std::static_pointer_cast<logical::project_relation>(root);
