...
bool gqe::optimizer::relation_properties::check_unique_key(
  std::vector<cudf::size_type> const& key) const noexcept
{
  auto sorted_key = key;
  std::sort(sorted_key.begin(), sorted_key.end());
  return std::find(_unique_keys.begin(), _unique_keys.end(), sorted_key) != _unique_keys.end();
}
