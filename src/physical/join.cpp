...
std::string join_relation_base::print() const
{
  ...
  // Like shift-and
  join_mem_string += "\t\"use like shift and\" : \"" +
                     std::string(use_like_shift_and() ? "enabled" : "disabled") + "\",\n";
...
std::string broadcast_join_relation::to_string() const
{
  ...
  // Hash map cache
  join_relation_string += "\t\"use hash map cache\" : \"" +
                          std::string(use_hash_map_cache() ? "enabled" : "disabled") + "\",\n";
  // Mark join
  join_relation_string +=
    "\t\"use mark join\" : \"" + std::string(use_mark_join() ? "enabled" : "disabled") + "\",\n";
  // Children
  join_relation_string +=
