      ...
      switch (cmd.op) {
        ...
        case ddl_command::operation::create_table: {
          ...
          _catalog->register_table(
            cmd.table_name, columns, cmd.storage, partitioning_schema_kind::none{}, unique_keys);
...
//   - "storage_kind" names a known in-memory variant
//   - no unrecognised keys appear alongside the chosen variant
//
// When a per-variant parameter is omitted, the variant's default constructor is used.
//
  ...
  google::protobuf::Map<std::string, gqe::proto::StorageOptionValue> const& opts)
{
  ...
  throw std::invalid_argument(std::format(
    ...
    "shared_numa_pool_memory, numa_memory, numa_pinned_memory",
    kind));
}
