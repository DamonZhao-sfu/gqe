...
use datafusion::sql::sqlparser::ast::{
    ColumnOption, CopyOption, CopySource, CopyTarget, CreateTableOptions, Expr as SqlExpr,
    ObjectType, Set, SqlOption, Statement as SqlStatement, TableConstraint, Value, ValueWithSpan,
};
    ...
    for stmt in statements {
        match &stmt {
            ...
            SqlStatement::CreateTable(create) if create.query.is_none() => {
                ...
                // DataFusion's planner does not implement CREATE TABLE WITH (...) at all
                // (it returns NotImplemented for any WITH clause, regardless of keys), so
                // strip the WITH clause before re-serializing for ctx.sql.
                ...
                let storage_options = extract_with_options(&create.table_options)?;
...
mod tests {
    ...
    fn parse_with_options(sql: &str) -> CreateTableOptions {
        ...
        match stmts.remove(0) {
            SqlStatement::CreateTable(ct) => ct.table_options,
