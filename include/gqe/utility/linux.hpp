...
/**
 * @brief Set the current thread's affinity to the process-wide CPU mask.
 *
 * The mask is read from the kernel via sched_getaffinity on the first call and
 * cached for the lifetime of the process. Call before any thread pinning to
 * ensure the cached mask reflects the initial set of available CPUs.
 */
