...
class cpu_set {
 public:
  ...
  static constexpr std::size_t byte_count =
    qword_count * sizeof(unsigned long); /**< Size of the cpu_set in bytes. */
...
struct table_statistics {
   ...
   */
  table_statistics(int64_t num_rows_, std::span<cudf::data_type const> column_types);
...
/**
 * @brief Uniform to_string / from_string interface for optimization-parameter enum types.
 *
 * Any enum field in @c optimization_parameters with a corresponding overload pair is
 * automatically handled by the serialization layer without additional registration.
 */
