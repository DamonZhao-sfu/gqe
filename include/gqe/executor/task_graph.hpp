...
struct task_graph {
  ...
  /**
   * @brief Write statistics collected during execution.
   *
   * Populated by the task_graph_builder for write plans. The statistics
   * manager is passed to write tasks during graph construction and filled
   * during execution. After execution completes, callers can read the
   * accumulated statistics via the manager's statistics() method.
   *
   * Null for non-write plans.
   */
  std::unique_ptr<table_statistics_manager> write_statistics;
};
...
class task_graph_builder {
 public:
   ...
   * info.
   * @param[in] factory A function that creates storage::table objects from descriptors.
   */
  task_graph_builder(context_reference ctx_ref, storage::table_provider const* provider);
 ...
 private:
  ...
  storage::table_provider const* _table_provider;
  ...
  std::unique_ptr<table_statistics_manager> _write_statistics;
};
