 ...
 *  - Unique key-sets (single-column and composite): `add/remove/check_unique_key`,
 *    `unique_keys`.
 */
class relation_properties {
 public:
  ...
  /**
   * @brief Return true if exactly this key-set (after sorting) is registered.
   */
  bool check_unique_key(std::vector<cudf::size_type> const& key) const noexcept;
