...
class zone_map_expression_transformer : public gqe::expression_visitor {
 public:
   ...
   * not be transformed.
   */
  static std::optional<arrow::compute::Expression> transform(
 ...
 private:
  ...
  std::unordered_map<const gqe::expression*, std::optional<arrow::compute::Expression>>
    _transformation_map{};
...
class zone_map {
 ...
 private:
  ...
  /// Compute the zone map as an Arrow RecordBatch in CPU memory.
  [[nodiscard]] static std::shared_ptr<arrow::RecordBatch> compute_arrow_zone_map(
  ...
  /// The zone map as an Arrow RecordBatch that lives on host memory.
  std::shared_ptr<arrow::RecordBatch> _zone_map;
...
class shared_zone_map : public zone_map {
 public:
  ...
  void load_from_shared_memory();

  std::vector<partition> evaluate(const arrow::compute::Expression& partial_filter) override;
...
/**
 * @brief Holds a serialized Arrow IPC stream in boost interprocess shared memory.
 *
 * On construction (rank 0), the RecordBatch is serialized into the IPC stream format and the
 * resulting bytes are copied into the shared memory segment.  Any rank can then call
 * to_record_batch() to deserialize back to an Arrow RecordBatch with near-zero-copy reads (Arrow
 * slices directly into the shared memory buffer).
 */
class shared_arrow_batch {
 public:
  shared_arrow_batch(const arrow::RecordBatch& batch,
                     boost::interprocess::managed_shared_memory& segment);

  ~shared_arrow_batch();

  [[nodiscard]] std::shared_ptr<arrow::RecordBatch> to_record_batch() const;

 private:
  boost::interprocess::offset_ptr<std::byte> _data;
  int64_t _size;
  boost::interprocess::managed_shared_memory* _segment;
};
...
class shared_zone_map_table {
 public:
  explicit shared_zone_map_table(const cudf::table_view& table,
                                 cudf::size_type partition_size,
                                 boost::interprocess::managed_shared_memory* segment);

  ~shared_zone_map_table();
  /// Serialized Arrow IPC zone map in shared memory.
  boost::interprocess::offset_ptr<shared_arrow_batch> _zone_map;
